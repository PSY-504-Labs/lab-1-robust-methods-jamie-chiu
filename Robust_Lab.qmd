...
2.  For the following data: \[8.453532, 10.025041, 11.495339, 9.367600, 8.333229, 9.788753, 10.883344, 10.543059, 9.869095, 10.799819\]
    ...
    ```{r}
    # create list
    my_list <- as.numeric(list(8.453532, 10.025041, 11.495339, 9.367600, 8.333229, 9.788753, 10.883344, 10.543059, 9.869095, 10.799819))

    # set seed
    set.seed(8)

    # using `simpleboot` package
    bootstrap_my_list <- one.boot(my_list,mean, R=1000)
    hist(bootstrap_my_list)
    ...
    boot_my_list <- boot(my_list, mean_func, R=1000)
    # get means
    boot_means <- boot_my_list$t

    # plot histogram with ggplot
    ggplot(as.data.frame(boot_means), aes(x = V1)) + 
      geom_histogram(binwidth = 0.1, 
                     color = "black", 
                     fill = "#1E88E5", # color-blind friendly
                     alpha = 0.9) +
      labs(title="Fig.2: Distribution of bootstrapped means",
           y="Frequency",
